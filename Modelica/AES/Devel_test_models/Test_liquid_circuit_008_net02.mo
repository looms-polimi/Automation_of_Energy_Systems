...
model Test_liquid_circuit_008_net02
  ...
  Modelica.Blocks.Sources.RealExpression cmdP(y = if time < 3600 then 0.25 elseif time < 2*3600 then 0.5
   elseif time < 3*3600 then 0.75 else 0.2) annotation(
    Placement(visible = true, transformation(origin = {-230, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression cmdQ(y = 100000) annotation(
...
protected
  annotation(
    experiment(StartTime = 0, StopTime = 18000, Tolerance = 1e-06, Interval = 1.8),
