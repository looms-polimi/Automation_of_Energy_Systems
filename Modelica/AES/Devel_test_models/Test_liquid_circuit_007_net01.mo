...
model Test_liquid_circuit_007_net01
  ...
  Modelica.Blocks.Sources.RealExpression cmdP(y = if time < 500 then 0.1 elseif time < 2000 then 0.6    elseif time < 3000 then 0.9 else 0.1) annotation(
  ...
  AES.ProcessComponents.Thermal.Liquid.Pump_centrifugal pumpC(w0 = 5)  annotation(
  ...
  inner ProcessComponents.Thermal.System_settings.System_terrain terrain annotation(
    Placement(visible = true, transformation(origin = {-218, 180}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
...
protected
  annotation(
    experiment(StartTime = 0, StopTime = 4000, Tolerance = 1e-6, Interval = 0.4),
...
end Test_liquid_circuit_007_net01;