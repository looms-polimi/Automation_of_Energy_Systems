...
model Test_header_001
  ...
  AES.ProcessComponents.Thermal.Liquid.dp_linear dp_linear annotation(
    Placement(visible = true, transformation(origin = {30, -22}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ProcessComponents.Thermal.Liquid.Node_wT_fixed src(T = 323.15)  annotation(
    Placement(visible = true, transformation(origin = {-102, -20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
  connect(header.pwh_b, dp_linear.pwh_a) annotation(
    Line(points = {{-4, -22}, {18, -22}}, color = {46, 52, 54}));
  connect(dp_linear.pwh_b, pref.pwh_a) annotation(
    Line(points = {{42, -22}, {78, -22}}, color = {46, 52, 54}));
